...
If this project later adds a small backend (a scheduled job plus a database), the natural next step — mirroring World Monitor's architecture — would be to poll FIRMS' NRT fire API, OpenAQ's v3 API, and GFW's GLAD alerts server-side, cache the results, and serve them to this page over a same-origin API. World Monitor's code is AGPL-3.0 licensed; any reuse of its source would need to comply with that license (including releasing derivative source code).
